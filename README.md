...
2. **첫 실행 — 열기 허용 (한 번만, 터미널 불필요)** — 무료 개인 개발 앱이라 **Apple 공증(notarization)이 없어** 처음엔 macOS가 한 번 막습니다. 아래로 허용하면 됩니다:
   - 앱을 **더블클릭** → *"확인되지 않은 개발자라서 열 수 없습니다"* 가 뜨면 **완료/취소**를 누릅니다. (정상 — 한 번 시도해야 다음 단계의 버튼이 생깁니다.)
   - **시스템 설정 → 개인정보 보호 및 보안** 을 열고 맨 아래로 스크롤 → **"'VtuberTranslate JP→KO'을(를) 차단했습니다 …" 옆의 〔그래도 열기〕** 버튼 클릭 → 암호 입력 → 다시 뜨는 창에서 **〔열기〕**.
   - 이후로는 그냥 더블클릭으로 열립니다. ✅

   > 혹시 위 버튼이 안 보이고 **"손상되어 열 수 없습니다"** 만 뜬다면 → **터미널**(응용 프로그램→유틸리티)에서 `xattr -dr com.apple.quarantine ` 입력 후 앱을 창으로 드래그 → **Enter**. (이때 "개발자 도구 설치?" 창이 뜨면 "설치" — Apple 정품, 안전.) 그래도 안 되면 [이슈](../../issues)로 알려주세요.
