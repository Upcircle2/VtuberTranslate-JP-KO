...
1. **다운로드** — [Releases](../../releases)에서 최신 `VtuberTranslate-JP-KO.zip`을 받아 압축을 풀고, 나온 앱(파일명 `VtuberTranslate.app`, Finder에는 **VtuberTranslate JP→KO**로 표시)을 **응용 프로그램** 폴더로 옮깁니다.

2. **첫 실행 (중요)** — 무료 개인 개발 앱이라 **Apple 공증(notarization)이 없어** 그냥 더블클릭하면 macOS가 "확인되지 않은 개발자" 또는 "손상되었다"며 막습니다. **한 번만** 잠금을 풀어주세요:
   - **터미널**(응용 프로그램 → 유틸리티)을 열고 아래 한 줄을 붙여넣어 실행:
     ```bash
     xattr -dr com.apple.quarantine /Applications/VtuberTranslate.app
     ```
   - 그다음 앱을 더블클릭하면 열립니다. (이후로는 그냥 열려요.)
